...
import {
    ...
    // overTimepApplication,

    // logTMSApplication,
    // privateInfoEI,
    // editTaxEI,
    // degreeEI,
    // approveDetailBusinessTripApplication,
    // addressPermanentEI,
    // addressTemporaryEI,
    // approveDetailLeaveApplication,
    // approveDetailLogTMSApplication,
    // approveDetailOverTimeApplication,
    // passPortEI,

} from '../custom/form/FormTypes'
...
export default class LeaveApplication extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
        ...
        for (let item of form) {
            if (item.tag === 'LeaveTypeID') {
                if (!stringIsEmpty(item.data)) {
                    ...
                    flag = false
                    Alert.alert(userProfile.LangID === 'VN' ? 'Thông báo' : 'Notice', userProfile.LangID === 'VN' ? 'Bạn chưa chọn loại nghỉ' : 'Choose type leave, please !', [{ text: 'Đóng', onPress: () => { } }])
                    break
                }
                ...
                if (item.value1 !== item.value2) {
                    ...
                    if (result) {
                        ...
                        flag = false
                        Alert.alert(userProfile.LangID === 'VN' ? 'Thông báo' : 'Notice', 'Từ ngày phải nhỏ hơn hoặc bằng đến ngày', [{ text: 'Đóng', onPress: () => { } }])
                    }
                    break
                }
            }
        ...
        if (!arrayIsEmpty(this.state._leaveApplication)) {
            ...
            for (let item of list) {
                if (item.control === 'twoButtonForm') {
                    ...
                    item.onPress = this.showDetailLeave
    ...
    componentDidUpdate(prevProps) {
        ...
        if (dataUpdateLeaveApplication !== prevProps.dataUpdateLeaveApplication) {
            if (!stringIsEmpty(dataUpdateLeaveApplication)) {
                Alert.alert('Thông báo', this.props.dataUpdateLeaveApplication, [{
                    text: 'Đóng', onPress: () => {
                        this.props.navigation.goBack()
                    }
                }])
        ...
        if (dataDeleteLeaveApplication !== prevProps.dataDeleteLeaveApplication) {
            if (!stringIsEmpty(dataDeleteLeaveApplication)) {
                Alert.alert('Thông báo', this.props.dataDeleteLeaveApplication, [{
                    text: 'Đóng', onPress: () => {
                        this.props.navigation.goBack()
                    }
                }])
                        ...
                        for (let item of form) {
                            if (item.tag === 'TotalLeaveHours') {
                                if (!stringIsEmpty(dataCal.gioNghi)) {
                                    ...
                                    item.value = time[0] + ' giờ ' + time[time.length - 1] + ' phút'
                                    item.data = dataCal.gioNghi
                                }
                            } else if (item.tag === 'TotalLeaveDays') {
                                item.value = dataCal.tongSoNgayNghi
                            } else if (item.tag === 'Approver') {
        ...
        if (this.props.errorCalculate !== prevProps.errorCalculate) {
            if (this.props.errorCalculate !== undefined) {
                Alert.alert('Thông báo', this.props.errorCalculate, [{ text: 'Đóng', onPress: () => { } }])
                this.setState({
                    isClickShowInfoLeave: false
                })
            }
        ...
        if (this.props.dataAttachFile !== prevProps.dataAttachFile) {
            if (this.props.dataAttachFile !== undefined) {
                Alert.alert('VN' ? 'Thông báo' : 'Notice', this.props.dataCreate, [{
                    text: userProfile.LangID === 'VN' ? 'Đóng' : 'Cancel', onPress: () => {
                        this.setState({
                            _leaveApplication: leaveApplication()
                        }, () => {
        ...
        if (this.props.errorAttachFile !== prevProps.errorAttachFile) {
            if (this.props.errorAttachFile !== undefined) {
                Alert.alert('VN' ? 'Thông báo' : 'Notice', this.props.errorAttachFile, [{ text: userProfile.LangID === 'VN' ? 'Đóng' : 'Cancel', onPress: () => { } }])
            }
        }
    ...
    onPressCalculateApplication = () => {
        ...
        let obj = {
            ...
            IsFromTom: '',
            IsToTom: '',
            LeaveRecordID: !objectIsNull(this.state.oldLeaveApplication) ? this.state.oldLeaveApplication.leaveRecordHourID : '',
            LeaveTypeID: '',
            ToDate: '',
            ToTime: '1',
        }
        for (let item of form) {
            if (item.control === 'twoDatePicker') {
                ...
                if (!stringIsEmpty(item.data)) {
                    ...
                    Alert.alert(userProfile.LangID === 'VN' ? 'Thông báo' : 'Notice', userProfile.LangID === 'VN' ? 'Vui lòng chọn loại nghỉ phép !' : 'Please select a leave type !', [{ text: userProfile.LangID === 'VN' ? 'Đóng' : 'Cancel', onPress: () => { } }])
                    return
                }
            }
        }
    ...
    onPressSubmit = () => {
        ...
        if (!objectIsNull(this.state.oldLeaveApplication)) {
            ...
            if (this.state.oldLeaveApplication.statusID === 2) {
                let inputRollback = {
                    LeaveRecordID: this.state.oldLeaveApplication.leaveRecordHourID,
                    Status: '3',
                    Reason: '',
                }
                let form = this.state._leaveApplication
            ...
            <View style={{
                ...
                <Text style={{ marginVertical: Sizes.s20, fontSize: Sizes.s30, color: '#0984e3', textAlign: 'center' }}>
                    {!stringIsEmpty(errorTypesLeave)
                        ? errorTypesLeave
                        : !stringIsEmpty(errorGetDetail) ? errorGetDetail :
